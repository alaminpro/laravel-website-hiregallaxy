			...
			<div class="col-md-4">
				<div class="employer-detail-sidebar">
					<div class="d-flex justify-content-center mb-3">
						<img src="{{ App\Helpers\ReturnPathHelper::getUserImage(auth()->user()->id) }}" class="img img-fluid rounded-circle" style="width: 60%;">
					</div> 
					<div class="single-job-description mt-2">
						<h3 class="text-center">{{ auth()->user()->name }}</h3>
						@if(!empty(auth()->user()->company->category))
						<p class="text-yellow mb-3 text-center"> {{ auth()->user()->company->category->name }}</p>
						@endif
					</div>
