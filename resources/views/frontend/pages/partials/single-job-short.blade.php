<div class="single-job-short single-employer">
		...
		<p class="mt-2">
			...
			@if (Auth::check())
				@if (Auth::user()->hasAppliedJob($single_job->id))
					<a href="#update-apply-job-modal" data-toggle="modal" class="btn btn-outline-success" onclick="applyUpdateJobDataSet({{ $single_job->id }}, '{{ $single_job->getCurrencyName() }}')">
						<span class="text-success"><i class="fa fa-check"></i> Already Applied</span>
					</a>
				@else
				<a href="#apply-job-modal" data-toggle="modal"  class="btn apply-now-button" onclick="applyJobDataSet({{ $single_job->id }}, '{{ $single_job->getCurrencyName() }}')">
				Apply Now
			</a>
				@endif
			@else
	...
	<div class="float-right text-right mb-3 ">
		...
		@if (Auth::check())
			...
			@else
			<a href="#apply-job-modal" data-toggle="modal" class="btn btn-outline-yellow" onclick="applyJobDataSet({{ $single_job->id }}, '{{ $single_job->getCurrencyName() }}')">
				Apply Now
			</a>
			@endif
		@else
