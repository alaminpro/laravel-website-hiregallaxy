...
class SegmentsController extends Controller
{
  ...
  public function store(Request $request)
  {
    ...
    session()->flash('success', 'Segment added successfully');
    return back();
  }
  ...
  public function destroy($id)
  {
    ...
    if ($segment) {
      if ($segment->status == 1) {
        ...
        $segment->status = 0;
        $segment->save();
        session()->flash('error', 'Segment trashed successfully');
      } else {
        $segment->delete();
        session()->flash('error', 'Segment deleted successfully');
      }
  ...
  public function active($id)
  {
    ...
    if ($segment) {
      $segment->status = 1;
      $segment->save();
      session()->flash('error', 'Segment Activated successfully');
      return redirect()->route('admin.segment.index');
    } else {
      return redirect()->route('admin.segment.index');
    }
